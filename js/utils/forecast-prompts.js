...
- "current_stage_id" is the FURTHEST stage the deal has substantially reached — the furthest stage that is either fully complete OR has at least half of its exit criteria met. This tracks the deal's real depth of progress: a deal three-quarters of the way through Development is a Development-stage deal even if one earlier checkbox is still open. Do NOT pin it to the last stage whose every last criterion is met when later stages are already well underway. Set "current_stage_confidence" to "low" when the notes are thin.
- "summary" must describe the deal at THIS current stage, consistent with the criteria you scored — do not narrate it as earlier (or later) than the evidence supports.
